...
public class Test {
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.out.println("耗时 "+cost +" ms! 参考：Intel I5-7200U 耗时700ms 左右!" );
			if(cost>1500) {
			...
			// 3
			PrimeUtil util2 = new PrimeUtil();
			//测试的时候 start、end、threadCount都有可能变化
			long start2 = 2000000L;//测试的时候可能为大于2的任何一个整数
			long end2 = 3000000L;//测试的时候 可能为大于start的任何一个整数
			int threadCount2 = 4;//测试的时候取值可能为[1,20]之间
			l=System.currentTimeMillis();
			//多线程计算
			List<Long> list2 = util.getPrimeList(start2, end2, threadCount2);
			//要求算法具有一定的效率速度
			...
			//测试的时候，会随机的从list中检查一些结果，本例子检查的是0,10001,20001...位置的质数
			long[] result2= {2000003,2145191,2291567,2439197,2586047,2734019,2882359};
			j=0;
			flag=true;
			for(int i=0;i<list2.size();i=i+10000) {
				if(result2[j]!=list2.get(i)) {
					System.out.print ("第"+i+"个质数错误!");
					flag=false;
				}
				j++;
